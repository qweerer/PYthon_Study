...
class Turtle(object):
    ...
    def eat(self):
        ...
        # the code below is moved to the main code, inorder to reduce a 'for cricle'
        '''
...
for i in range(10):
    ...
    '''
    fish=Fish("123")
    fish_list_die.append(fish)
    ''' 
    # you can creat different object use the same name,
    # but in this game, using the method below will show a clearn process
...
# using the method below will be more significance
    if tur.heart <= 0:
        print('乌龟死亡，鱼获胜')
        break
    if len(fish_list) == 0:
    ...
    tur.move()
    for fish_each in fish_list[:]:
        fish_each.move()
        if tur.x == fish_each.x & tur.y == fish_each.y:
            tur.eat()
            fish_list.remove(fish_each)
            fish_list_die.append(fish_each)
            fish_each.die()
            print('乌龟吃了{}'.format(fish_each.name))
